...
char *msg4[] = {
    "We've used both graphics pages",
...
char *argv[];
{
    ...
    if (!screen(3))
    {
        if (_gfx.card_monitor & 0xF00 == MDA_CARD)
            exit(1);
...
void graphics_suite()
{
    ...
    if (sizeof(char *) == 4)
    {
        ...
        if (_gfx.bios_mode >= HERC_GFX)
            display_message(msg4, 5);
        SPIN();
        get_fpic(0, 0, 319, 199, 0, fpic_handle);
        delay(30);
        cls();
...
void draw_triangle(x, y, base, height, color) int x, y, base, height, color;
{
    int y2, new_color;
...
void TRIANGLES()
{
    ...
    while (n_reps--)
    {
        if ((fill_color = color + 1) >= max_color)
            fill_color = 1;
...
void MOVE_BLOCK()
{
    ...
    pic1 = get_pic(99, 59, 159, 99);
    p1x = 99;
    p1y = 59;
    pic2 = get_pic(160, 59, 221, 100);
    p2x = 160;
    p2y = 59;
    pic3 = get_pic(99, 101, 159, 141);
    p3x = 99;
    p3y = 101;
    pic4 = get_pic(160, 101, 221, 141);
    p4x = 160;
    p4y = 101;
    max_x = 320;
    max_y = 200;
...
void SHOW_FPICS(fpic_name) char *fpic_name;
{
    int i, fpic_handle;
...
void FONT_TEST()
{
    ...
    puts_font(88, 70, box_color, "�������������ͻ");
    for (i = 1; i < 5; i++)
        puts_font(88, 70 + (i * dy), box_color, "�             �");
    puts_font(88, 70 + (5 * dy), box_color, "�������������ͼ");
    delay(30);
    for (i = 1; i < 140; i++)
    {
...
display_message(tp, n_lines) char **tp;
int n_lines;
{
...
void delay(n_ticks) int n_ticks;
{
    unsigned int ct;

    while (kbhit())
    {
        if ((_gfx_call_dos(7, 0) & 0xFF) == '*')
            restore_scrn();
    }
